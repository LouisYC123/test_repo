...
class TransformPreException(luigi.Task):
    ...
    def output(self):
        return luigi.LocalTarget(
            "data/transform_pre_exception/" + self.data_key + ".pkl",
            format=luigi.format.Nop,
        )
...
class TransfromPostException(luigi.Task):
    ...
    def run(self):
        data = {
            key: handler.read_staging("data/transform_pre_exception/" + key + ".pkl")
        ...
        for i in data:
            ...
            data[i] = merger.ExceptionHandler(
                data[i], project_config, client_exception, i
            ).run()
...
class FinaliseInScopeEmployee(luigi.Task):
    ...
    def output(self):
        return luigi.LocalTarget(
            "data/final_staging/" + self.data_key + ".pkl", format=luigi.format.Nop
        )
        ...
        employee_list = list(
            set(
                [
                    item
                    for sublist in [
                        out_of_scope_employee[i]["EmployeeCode"].tolist()
                        for i in out_of_scope_employee
                    ]
                    for item in sublist
                ]
            )
...
class Load(luigi.Task):
    ...
    def run(self):
        if self.data_key == "Configuration":
            ...
            data = handler.read_staging(
                "data/final_staging/" + self.config[self.data_key]["source"] + ".pkl"
            )
