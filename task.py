...
class TransformPreException(luigi.Task):
    ...
    def output(self):
        return luigi.LocalTarget('data/transform/'+self.data_key+'.pkl',format=luigi.format.Nop)
...
class TransfromPostException(luigi.Task):
    ...
    def run(self):
        ...
        '''
        data={key : handler.read_staging('data/transform/'+key+'.pkl') for key in self.config['source']}
        for i in data:
...
class Load(luigi.Task):
    ...
    def run(self):
        if self.data_key=='Configuration':
            ...
            data=handler.read_staging('data/transform_post_exception/'+self.config[self.data_key]['source']+'.pkl')
...
class RunLoad(luigi.Task):
    ...
    def output(self):
        return luigi.LocalTarget('data/flag/sentinel_COMPLETE.txt',format=luigi.format.Nop)
